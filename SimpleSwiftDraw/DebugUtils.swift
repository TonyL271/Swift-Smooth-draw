...
extension Logger {
    func ilog(_ items: Any..., file: String = #file, lineNum: Int = #line) {
        let fileName = (file as NSString).lastPathComponent
        let stringified = items.map { String(describing: $0) }.joined(separator: "")
        self.log("\(stringified) {file: \"\(fileName)\", line \(lineNum)}")
    }
}

public let logger = Logger(subsystem: "MetalDraw", category: "general")
